...
def render(
    ...
    try:
        ...
        print("Run: cd templates/vault/.agents/skills/excalidraw/references && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run: cd templates/vault/.agents/skills/excalidraw/references && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
        ...
        # Wait for the ES module to load (imports from esm.sh) - use longer timeout for first load
        # Also check that renderDiagram function exists (module loaded)
        page.wait_for_function("window.__moduleReady === true && typeof window.renderDiagram === 'function'", timeout=90000)
